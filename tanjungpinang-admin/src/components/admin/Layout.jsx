...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      {/* Mobile drawer */}
      <aside className={cn(
        "fixed inset-y-0 left-0 z-50 w-56 bg-white border-r border-gray-100 shadow-xl transition-transform duration-300 md:hidden",
        mobileOpen ? "translate-x-0" : "-translate-x-full"
      )}>
        <button onClick={() => setMobileOpen(false)} className="absolute top-3 right-3 p-1 rounded-md text-gray-400 hover:text-gray-600">
          <X size={16} />
        </button>
      ...
      <aside className={cn(
        ...
        <SidebarContent collapsed={collapsed} setCollapsed={setCollapsed} onNavClick={() => {}} />
      </aside>
